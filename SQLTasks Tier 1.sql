...
'cheap' eor 'expensive', depending on if their monthly maintenance cost is
more than $100. Return the name and monthly maintenance of the facilities
in question. */
SELECT name, montlymaintenance,
CASE 
     WHERE montlymaintenance > 100 THEN 'expensive'
     ELSE 'cheap' 
END AS label
FROM `Facilities`;
...
SELECT facilities.name, firstname+surname AS fullname FROM 'members'
INNER JOIN 'bookings'
...
SELECT firstname|| ' ' || surname AS fullname, name,
...
SELECT members.surname//' '//members.firstname, recommendedby FROM members 
ORDER BY surname;

/* Q12: Find the facilities with their usage by member, but not guests */
SELECT name FROM bookings
INNER JOIN facilites
ON bookings.facid = facilities.facid
WHERE memdi !=0;

/* Q13: Find the facilities usage by month, but not guests */
SELECT COUNT(slots) FROM bookings
GROUP BY starttime.month AND facid
HAVING memid != 0；
